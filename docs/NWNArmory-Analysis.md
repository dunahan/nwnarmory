...
## 9. Modernization: Done, Rust CLI

The Rust CLI described in this section's earlier revisions is no longer a proposal: it is the project's only maintained implementation (`src/`, current release documented in the root `README.md` and `CHANGELOG.md`). Every anomaly listed in Section 6 above has been addressed in the rewrite (see `CHANGELOG.md`'s 1.2.1 and 1.3.3 entries for the specifics: collision detection, populated I/O errors, atomic no-partial-write output, the `position=`/`setsupermodel` fixes). It runs natively as a CLI on any platform Rust targets, including Linux, with no GUI and no MFC dependency.

Field-by-field coverage of both the original tool's documented behavior and the newer NWN:EE ASCII additions (`normals`, `tangents`, `materialname`, extra UV channels, `weights`/`constraints`, and what's still open) is tracked separately in [`docs/rust-port-format-coverage.md`](rust-port-format-coverage.md), so that detail doesn't go stale here every time the Rust side changes.
